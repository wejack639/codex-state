...
- 按工作空间分组展示已关注 Chat。
- 每个会话的 ☆ 按钮可切换「重点关注」金色星标；标记持续保留，打开会话、清除未查看提醒、跨天或重启不会取消，再点 ★ 可手动取消。移除会话时一并清除星标。
- 收起时保留透明动态小人与状态灯，悬停后展示全部关注项目与 Chat。
- 七套完整主题可一键切换，重点关注使用金星和淡主题色底，不覆盖红色未查看提醒。
- 识别「运行中」「已完成」「已中断」「可能断联」。
- 状态灯区分绿灯运行中、红灯完成未查看、灰灯完成已查看、黄灯状态未知（需要核实会话是否仍在运行）；红灯在悬浮入口上优先显示。
...
展开悬浮面板，点击顶部“收回 / 添加”旁的调色盘按钮，在面板内点选色卡即可立即切换并保存；勾选表示当前主题，不需要确认或重启。点击色卡外部或按 Esc 关闭选择器，随时可以切回其他主题。

内置冰川蓝、樱花粉、薄荷绿、雾霭紫、蜜桃橙、云朵白、午夜深色，共七套完整配色。卡片、按钮、搜索框、名称编辑框和悬停效果同步切换。旧配置默认使用午夜深色，不会升级后突然切为浅色；推荐搭配蓝发小人使用冰川蓝。

主题以 `theme` 字段保存在 `~/.codex-state/config.json`，与关注列表、重点关注和已查看时间一起保存。添加、移除、标星、打开会话、状态刷新及重启均不会重置主题。保存失败时会显示错误并恢复已保存的配色。浏览器版与悬浮窗口通过本机桥接同步选择。

主题不会改变状态含义：绿灯运行中、黄灯未知、红灯完成未查看、灰灯完成已查看；重点关注的金星独立保留。悬浮小人的透明边缘、尺寸及皮肤动画也不随主题变化。
...
- 关注列表、重点关注、已查看时间与主题保存在 `~/.codex-state/config.json`。
